...
def davinz_score(theta, y_predict, y_true, evaluate_x, test_x, sigma):
    """
    Computes the DaVinz score.
    
    Args:
        theta (Tensor): parameters of the model
        y_predict (Tensor): predictions of the model
        y_true (Tensor): true labels
        evaluate_x (Tensor): data used to compute MMD
        test_x (Tensor): data used to compute MMD
        sigma (float): lengthscale of the RBF kernel
        
    Return:
        DaVinz score
    """
    mmd_score = rbf_mmd2(evaluate_x, test_x, sigma)
