...
def load_pretrained_feature_extractor(feature_extractor_name, device):
    ...
    net_test.load_state_dict(torch.load('LAVA/checkpoint/'+feature_extractor_name))
    net_test.eval()
    return net_test
...
def compute_dual_no_vis(feature_extractor, trainloader, testloader, p=2, resize=32, device='cuda'):
    # OT Dual calculation
    dual_sol = get_OT_dual_sol(feature_extractor, trainloader, testloader, p=2, resize=32, device='cuda')
    return dual_sol
...
# Get the data values
def compute_values(dual_sol, training_size):
    calibrated_gradient = values(dual_sol, training_size)
    return calibrated_gradient
