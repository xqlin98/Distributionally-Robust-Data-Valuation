...
from utils.models_defined import CNN, MLP_REGRESSION, MLP_REGRESSION_L, MLP_REGRESSION_S, MLP_CLS_L, MLP_CLS_S, Model_Train
...
if __name__ == "__main__":
    ...
    flatten = False if "CNN" in model else True
    save = True if dataset in ["mnist", 'cifar10'] else False
    save_name = f'Subset_Selection/{dataset}-numdp-{numdp}-seed-{seed}-model-{model}-epsilon-{epsilon}-{expname}-allbaseline.npz'
    ...
    if cmd_args['restore']:
        ...
        if_scores_m, trackin_scores_m, drge_if_score_m, drge_trackin_score_m, drge_random_score_m, drge_if_score_l2h_m, drge_trackin_score_l2h_m, drge_random_score_l2h_m = [tmp for tmp in result_restore['if_scores_m']], [tmp for tmp in result_restore['trackin_scores_m']], [tmp for tmp in result_restore['drge_if_score_m']], [tmp for tmp in result_restore['drge_trackin_score_m']], [tmp for tmp in result_restore['drge_random_score_m']], [tmp for tmp in result_restore['drge_if_score_l2h_m']], [tmp for tmp in result_restore['drge_trackin_score_l2h_m']], [tmp for tmp in result_restore['drge_random_score_l2h_m']]
    ...
    for round in range(cmd_args['trial']):
        ...
        # load data
        if not save:
            trdata, tedata, trlabel, telabel = load_data(dataset=dataset, numdp=numdp, flatten=flatten, seed=tmp_seed)
        else:
            trdata, tedata, trlabel, telabel, trdata_raw, trlabel_raw = load_data_save(dataset=dataset, numdp=numdp, flatten=flatten, seed=tmp_seed)    
        trdata, tedata = torch.tensor(trdata, dtype=torch.float32).to(device), torch.tensor(tedata, dtype=torch.float32).to(device)

        # nn trainging on grand dataset
        ...
        gpu_sequnce = gpus * (numdp // len(gpus) + 1)
        
        tmp_idxs = np.array_split(np.arange(numdp), thread_num_nn)
        scores_result = Parallel(n_jobs=thread_num_nn, max_nbytes=5000)(delayed(compute_drge_score_for_one)(tmp_idxs[tmp], gpu_sequnce[tmp], show=(tmp==0), desc=f"Removing sets for loo and drge, round {round}") for tmp in range(thread_num_nn))
        scores_result = np.vstack(scores_result)
        loo_scores, marginal_drge = scores_result[:,0], scores_result[:,1]
        
        with tqdm_joblib(tqdm(desc=f"Removing sets for deviation score, round {round}", total=numdp)) as progress_bar:
            scores_result = Parallel(n_jobs=thread_num, max_nbytes=5000)(delayed(compute_deviation_score_for_one)(tmp, regression=regression, class_num=class_num) for tmp in range(numdp))
        deviation_scores = np.array(scores_result)
        # deviation_scores=loo_scores=marginal_drge = np.zeros(numdp)
        
        deviation_scores_m += [deviation_scores]
        loo_scores_m += [loo_scores]
        marginal_drge_m += [marginal_drge]
        if_scores_m += [influence_scores]
        trackin_scores_m += [trackin_scores]
        if not regression:
            lava_scores_m += [lava_scores]
        davinz_scores_m += [davinz_scores]

        def remove_evaluation(set_idx, device, train_set, regression=True):
            set_selected = np.array(list(set(range(numdp)) - set(set_idx)))
        ...
        remove_percentages = np.arange(0.1,1,0.05)
    
        # data removal using deviation score
        sorted_idx = np.argsort(deviation_scores)[::-1]
        with tqdm_joblib(tqdm(desc=f"Evaluation deviation, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_deviation_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_deviation_score_m += [drge_deviation_score]
        
        # data removal using loo score
        sorted_idx = np.argsort(loo_scores)[::-1]
        with tqdm_joblib(tqdm(desc=f"Evaluation LOO, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_loo_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_loo_score_m += [drge_loo_score]
        
        # data removal using influence function
        sorted_idx = np.argsort(influence_scores)[::-1]
        with tqdm_joblib(tqdm(desc=f"Evaluation influence function, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_if_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_if_score_m += [drge_if_score]
        
        # data removal using trackin score
        sorted_idx = np.argsort(trackin_scores)[::-1]
        with tqdm_joblib(tqdm(desc=f"Evaluation trackin, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_trackin_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_trackin_score_m += [drge_trackin_score]

        # data removal using lave score, only when classification
        if not regression:
            sorted_idx = np.argsort(lava_scores)[::-1]
            with tqdm_joblib(tqdm(desc=f"Evaluation lava, round {round}", total=len(remove_percentages))) as progress_bar:
                drge_lava_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
            drge_lava_score_m += [drge_lava_score]
        
        # data removal using davinz score
        sorted_idx = np.argsort(davinz_scores)[::-1]
        with tqdm_joblib(tqdm(desc=f"Evaluation davinz, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_davinz_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_davinz_score_m += [drge_davinz_score]

        # data removal using random
        sorted_idx = np.random.permutation(range(numdp))
        with tqdm_joblib(tqdm(desc=f"Evaluation random, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_random_score = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation deviation L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_deviation_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_deviation_score_l2h_m += [drge_deviation_score_l2h]

        # data removal using loo score l2h
        sorted_idx = np.argsort(loo_scores)
        with tqdm_joblib(tqdm(desc=f"Evaluation LOO L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_loo_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_loo_score_l2h_m += [drge_loo_score_l2h]

        # data removal using influence function l2h
        sorted_idx = np.argsort(influence_scores)
        with tqdm_joblib(tqdm(desc=f"Evaluation influence function L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_if_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_if_score_l2h_m += [drge_if_score_l2h]

        # data removal using trackin l2h
        sorted_idx = np.argsort(trackin_scores)
        with tqdm_joblib(tqdm(desc=f"Evaluation trackin L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_trackin_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_trackin_score_l2h_m += [drge_trackin_score_l2h]

        # data removal using lave score l2h, only when classification
        if not regression:
            sorted_idx = np.argsort(lava_scores)
            with tqdm_joblib(tqdm(desc=f"Evaluation lava L2H, round {round}", total=len(remove_percentages))) as progress_bar:
                drge_lava_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
            drge_lava_score_l2h_m += [drge_lava_score_l2h]
        
        # data removal using davinz score l2h
        sorted_idx = np.argsort(davinz_scores)
        with tqdm_joblib(tqdm(desc=f"Evaluation davinz L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_davinz_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
        drge_davinz_score_l2h_m += [drge_davinz_score_l2h]
        
        # data removal using random l2h
        sorted_idx = np.random.permutation(range(numdp))
        with tqdm_joblib(tqdm(desc=f"Evaluation random L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_random_score_l2h = Parallel(n_jobs=thread_num_nn)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)], gpu_sequnce[k], train_set,regression) for k,tmp in enumerate(remove_percentages))
