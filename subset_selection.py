...
if __name__ == "__main__":
    ...
    save_name = f'Subset_Selection/{dataset}-numdp-{numdp}-seed-{seed}-length_scale-{length_scale}-epsilon-{epsilon}-mu-{mu}-{expname}.npz'
    ...
    for round in range(cmd_args['trial']):
        ...
        # load data
        if not save:
            trdata, tedata, trlabel, telabel = load_data(dataset=dataset, numdp=numdp, seed=tmp_seed)
        else:
            trdata, tedata, trlabel, telabel, trdata_raw, trlabel_raw = load_data_save(dataset=dataset, numdp=numdp, seed=tmp_seed)    
        ...
        with tqdm_joblib(tqdm(desc=f"Removing sets, round {round}", total=numdp)) as progress_bar:
            scores_result = Parallel(n_jobs=thread_num)(delayed(compute_scores_for_one)(tmp, regression=regression, class_num=class_num) for tmp in range(numdp))
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation deviation, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_deviation_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation LOO, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_loo_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation influence function, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_if_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation trackin, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_trackin_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        if not regression:
            ...
            with tqdm_joblib(tqdm(desc=f"Evaluation lava, round {round}", total=len(remove_percentages))) as progress_bar:
                drge_lava_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation davinz, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_davinz_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        drge_davinz_score_m += [drge_davinz_score]

        # data removal using random
        sorted_idx = np.random.permutation(range(numdp))
        with tqdm_joblib(tqdm(desc=f"Evaluation trackin, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_random_score = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation deviation L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_deviation_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation LOO L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_loo_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation influence function L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_if_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation trackin L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_trackin_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        if not regression:
            ...
            with tqdm_joblib(tqdm(desc=f"Evaluation lava L2H, round {round}", total=len(remove_percentages))) as progress_bar:
                drge_lava_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation davinz L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_davinz_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
        ...
        with tqdm_joblib(tqdm(desc=f"Evaluation random L2H, round {round}", total=len(remove_percentages))) as progress_bar:
            drge_random_score_l2h = Parallel(n_jobs=thread_num)(delayed(remove_evaluation)(sorted_idx[:int(numdp * tmp)],regression) for tmp in remove_percentages)
